...
if(program.server) {
    ...
    const server = net.createServer((c) => {
        ...
        c.on('data', (d) => {
            ...
            const decryptedJSON = decryptPub(pubKey, d);
            const decryptedData = JSON.parse(decryptedJSON);
            print(`${decryptedData.data}\n`);
            print('--------\n');
            const dataReply = `Dear client, the server has received the data on ${Date().toString()}`;
            c.write(JSON.stringify({
                data: encryptPub(pubKey, dataReply),
                type: 'data',
                checksum: ''
            }));
        });
        ...
        print(`${randomBytes}\n`);
        const dataBack = `Hello from server! - ${Date().toString()}\n` + 
                         `Random string generated by server: ${randomBytes}`;
        c.write(JSON.stringify({
            data: encryptPub(pubKey, dataBack),
            type: 'data',
            checksum: ''
        }));
    });
    ...
    client.on('data', (d) => {
        ...
        print(`${d.toString('hex')}\n`);
        print('Decrypted: ', 'yellow');
        const decryptedJSON = decryptPriv(privKey, passphrase, d);
        const decryptedData = JSON.parse(decryptedJSON);
        print(`${decryptedData.data}\n`);
        print('--------\n');
    });
